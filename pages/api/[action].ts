...
import {
  getUsersHandler,
  getUsersByPlanHandler,
} from "../../controllers/userController";
import {
  cancelPostScheduleHandler,
  generatePostTopicsHandler,
  getPostTopicsHandler,
...
export default async function handler(
  ...
  } else if (req.method === "POST" && action === "cancel-post-schedule") {
    await authenticate(cancelPostScheduleHandler)(req, res);
  } else {
    res.setHeader("Allow", ["GET"]);
    res
      .status(405)
      .json({ message: `Method ${req.method} Not Allowed or Invalid Action` });
  }
}
